...
editNote() {
 ...
 this.notes=this.notes.map(note=> note.id===Number(this.id) ? {
   ...note, title, text
  }
...
selectNote(event) {
 ...
 const [$noteTitle,
 $noteText]=$selectedNote.children;
...
displayNotes() {
 ...
 this.$notes.innerHTML =this.notes.map(note=> ` <div style="background: ${note.color};" class="note" data-id="${note.id}" > <div class="${note.title && 'note-title'}" >${note.title}</div> <div class="note-text" >${note.text}</div> <div class="toolbar-container" > <div class="toolbar" > <em class="toolbar-delete fas fa-trash-alt" data-id=${note.id}></em> </div> </div> </div> `).join(""); // to get rid of the commas between our arrays
...
function search_card() {
 let search=document.getElementById('find').value search=search.toLowerCase();
